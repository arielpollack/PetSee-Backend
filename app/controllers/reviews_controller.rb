class ReviewsController < ApplicationController
    ...
    def create
        r = review_params
        render_unprocessable_entity "rate must be between 1 to 5" and return if not (1..5).include? r[:rate].to_i
        render_not_found "feedback must contain text" and return if r[:feedback].nil? || r[:feedback] == ""

        @review = Review.where(writer_id: @current_user.id, user_id: params[:user_id]).first
        Rails.logger.info "IS NEW REVIEW? #{@review.nil?}"

        if @review.nil? # New review
            ...
            render_forbidden "You have already written a review about this user" and return
        end
        if @review.save
    ...
    def update_existing_review
        ...
        render_forbidden "You are not the writer of this review" and return if not @review.writer_id == @current_user.id
        render_not_found "There is no review of this number" and return if @review.nil?
